...
selected_data_fromschool <- fromschool_data %>% filter(schoolindex == 1) %>% group_by(school) %>% mutate(tot = sum(y16)) %>% arrange(desc(tot)) %>% distinct(school,tot)
...
p <- p+coord_polar('y', start = 0) + ggtitle(2016)
