...
insert_chart_foreign <- function(school_index, year_value){
  ...
  foreign <- raw_data_foreign %>% filter(schoolindex == school_index) %>% select(selected_colname_foreign)
...
# 본문 시작
  ...
  raw_data_table_part1 <- read.csv('./rawdatas/abc_part1.csv')
  raw_data_table_part2 <- read.csv('./rawdatas/abc_part2.csv')
